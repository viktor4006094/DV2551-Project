...
void Project::SetResourceTransitionBarrier(
...
void Project::CreateRenderShadersAndPiplelineState()
{
	////// Shader Compiles //////
	ID3DBlob* vertexBlob;
	D3DCompileFromFile(
...
void Project::CreateComputeShadersAndPiplelineState()
{
	////// Shader Compiles //////
...
void Project::CreatePassthroughShadersAndPiplelineState()
{
	////// Shader Compiles //////
	ID3DBlob* vertexBlob;
	D3DCompileFromFile(
...
void Project::ComputePass(int index)
{
	UINT backBufferIndex = gSwapChain4->GetCurrentBackBufferIndex();
	//Command list allocators can only be reset when the associated command lists have
	//finished execution on the GPU; fences are used to ensure this (See WaitForGpu method)
...
void Project::PassthroughPass(int index)
{
	UINT backBufferIndex = gSwapChain4->GetCurrentBackBufferIndex();
	//Command list allocators can only be reset when the associated command lists have
	//finished execution on the GPU; fences are used to ensure this (See WaitForGpu method)
